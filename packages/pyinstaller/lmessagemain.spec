...
srcPath = "C:\\src\\PyLMessage\\src\\"
qtPath = "C:\\Qt-4.8.7\\"
openSSLPath = "C:\\OpenSSL-1.0.2j\\bin\\"
outPath = "C:\\src\\PyInstaller-3.2.1\\lmessagemain"
...
# -*- mode: python -*-
a = Analysis([srcPath + 'lmessagemain.py'],
             pathex=[outPath],
             hiddenimports=[],
             hookspath=None,
             runtime_hooks=None)
...
def addUIs():
    import os
    extraDatas = []
    for file in os.listdir(srcPath + 'lmessageqt'):
        if file[-3:] != ".ui":
            continue
        extraDatas.append((os.path.join('ui', file), os.path.join(srcPath, 'lmessageqt', file), 'DATA'))
    return extraDatas
...
fname = 'LMessage_%s_%s.exe' % ("x86" if arch == 32 else "x64", softwareVersion)
if snapshot:
    fname = 'LMessagedev_%s_%s.exe' % ("x86" if arch == 32 else "x64", today)
