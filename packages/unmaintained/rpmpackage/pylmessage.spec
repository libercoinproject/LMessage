...
License: MIT
URL: https://github.com/LMessage/PyLMessage
...
%description
LMessage is a P2P communications protocol used to send encrypted messages
...
- Refactored of the way PyLMessage looks for interesting new objects
...
- Allow specification of alternate settings dir
  via LMESSAGE_HOME environment variable
...
- Added option in settings to allow sending to a mobile device
  (app not yet done)
- Added ability to start daemon mode when using LMessage as a module
...
- Add missing trailing semicolons to pylmessage.desktop
...
- On Linux, when selecting Show LMessage, do not maximize
  automatically
- Store tray icons in lmessage_icons_rc.py

* Sat May 25 2013 Jonathan Warren (4096 bits) <jonathan@lmessage.org> - 0.3.1-1
...
- Don't update the time we last communicated with a node unless
  ...
  forget about active but non-LMessage nodes which have made
...
- Bugfix: If address book label is blank, LMessage wouldn't
...
- Allow the cancelling of a message which is in the process of being
  sent by trashing it then restarting LMessage
...
- Fixed Ubuntu & OS X issue:
  LMessage wouldn't receive any objects from peers after restart.
- Inventory flush to disk when exiting program now vastly faster.
- Fixed address generation bug (kept LMessage from restarting).
