...
# Right now, PyLMessage only support connecting to stream 1. It doesn't
...
# Used to capture a Ctrl-C keypress so that LMessage can shutdown gracefully.
import signal
...
def connectToStream(streamNumber):
    ...
    try:
        ...
        if BMConfigParser().get(
                'lmessagesettings', 'socksproxytype') != 'none':
...
class singleAPI(threading.Thread, helper_threading.StoppableThread):
    ...
    def stopThread(self):
        ...
        try:
            s.connect((
                ...
                BMConfigParser().getint('lmessagesettings', 'apiport')
            ))
    ...
    def run(self):
        port = BMConfigParser().getint('lmessagesettings', 'apiport')
        try:
        ...
        for attempt in range(50):
            try:
                ...
                if attempt > 0:
                    BMConfigParser().set(
                        "lmessagesettings", "apiport", str(port))
                    BMConfigParser().save()
                break
...
class Main:
    def start(self):
        ...
        if state.enableGUI and not state.curses and not depends.check_pyqt():
            sys.exit(
                'PyLMessage requires PyQt unless you want'
                ...
                'https://lmessage.org/wiki/Daemon\n'
                'You can also run PyLMessage with'
        ...
        helper_threading.set_thread_name("PyLMessage")
        ...
        if state.dandelion and not BMConfigParser().safeGetBoolean(
                'lmessagesettings', 'sendoutgoingconnections'):
            state.dandelion = 0
        ...
        if not daemon and state.enableGUI:
            if state.curses:
                ...
                import lmessagecurses
                lmessagecurses.runwrapper()
            elif state.kivy:
                BMConfigParser().remove_option('lmessagesettings', 'dontconnect')
                from lmessagekivy.mpybit import NavigateApp
                NavigateApp().run()
            else:
                import lmessageqt
                lmessageqt.run()
...
'''
    ...
    def stop(self):
        with shared.printLock:
            print('Stopping LMessage Deamon.')
    ...
    def getApiAddress(self):
        if not BMConfigParser().safeGetBoolean(
                'lmessagesettings', 'apienabled'):
            return None
        address = BMConfigParser().get('lmessagesettings', 'apiinterface')
        port = BMConfigParser().getint('lmessagesettings', 'apiport')
...
# So far, the creation of and management of the LMessage protocol and this
