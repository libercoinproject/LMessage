...
"""
Created by Adam Melton (.dok) referenceing https://lmessage.org/wiki/API_Reference for API documentation
Distributed under the MIT/X11 software license. See http://www.opensource.org/licenses/mit-license.php.

This is an example of a daemon client for PyLMessage 0.6.2, by .dok (Version 0.3.1) , modified
...
def restartBmNotify():
    """Prompt the user to restart LMessage"""
    print '\n     *******************************************************************'
    print '     WARNING: If LMessage is running locally, you must restart it now.'
    print '     *******************************************************************\n'
...
def lookupAppdataFolder():
    """gets the appropriate folders for the .dat files depending on the OS. Taken from lmessagemain.py"""
...
def configInit():
    ...
    BMConfigParser().add_section('lmessagesettings')
    # Sets the lmessage port to stop the warning about the api not properly
    ...
    BMConfigParser().set('lmessagesettings', 'port', '8444')
    BMConfigParser().set('lmessagesettings', 'apienabled', 'true')  # Sets apienabled to true in keys.dat
...
def apiInit(apiEnabled):
    ...
    if apiEnabled is False:  # API information there but the api is disabled.
        ...
        if uInput == "y":
            ...
            print '     Please refer to the LMessage Wiki on how to setup the API.'
            print '     ************************************************************\n'
            usrPrompt = 1
            main()
        ...
        if uInput == "y":  # User said yes, initalize the api by writing these values to the keys.dat file
            ...
            # directory or is created locally to connect to a machine remotely.
            BMConfigParser().set('lmessagesettings', 'port', '8444')
            BMConfigParser().set('lmessagesettings', 'apienabled', 'true')
            BMConfigParser().set('lmessagesettings', 'apiport', apiPort)
            BMConfigParser().set('lmessagesettings', 'apiinterface', '127.0.0.1')
            BMConfigParser().set('lmessagesettings', 'apiusername', apiUsr)
            BMConfigParser().set('lmessagesettings', 'apipassword', apiPwd)
            BMConfigParser().set('lmessagesettings', 'daemon', daemon)
            with open(keysPath, 'wb') as configfile:
                BMConfigParser().write(configfile)
...
def apiData():
    ...
    try:
        ...
        try:
            ...
            print '\n     ******************************************************************'
            print '     There was a problem trying to access the LMessage keys.dat file'
            print '                    or keys.dat is not set up correctly'
            print '       Make sure that daemon is in the same directory as LMessage. '
            print '     ******************************************************************\n'
    ...
    try:  # checks to make sure that everyting is configured correctly. Excluding apiEnabled, it is checked after
        BMConfigParser().get('lmessagesettings', 'apiport')
        BMConfigParser().get('lmessagesettings', 'apiinterface')
    ...
    # apiEnabled =
    # apiInit(BMConfigParser().safeGetBoolean('lmessagesettings','apienabled'))
    ...
    apiPort = int(BMConfigParser().get('lmessagesettings', 'apiport'))
    apiInterface = BMConfigParser().get('lmessagesettings', 'apiinterface')
    apiUsername = BMConfigParser().get('lmessagesettings', 'apiusername')
    apiPassword = BMConfigParser().get('lmessagesettings', 'apipassword')
...
def bmSettings():
    ...
    try:
        port = BMConfigParser().get('lmessagesettings', 'port')
    except:
        print '\n     File not found.\n'
        usrPrompt = 0
        main()

    startonlogon = BMConfigParser().safeGetBoolean('lmessagesettings', 'startonlogon')
    minimizetotray = BMConfigParser().safeGetBoolean('lmessagesettings', 'minimizetotray')
    showtraynotifications = BMConfigParser().safeGetBoolean('lmessagesettings', 'showtraynotifications')
    startintray = BMConfigParser().safeGetBoolean('lmessagesettings', 'startintray')
    defaultnoncetrialsperbyte = BMConfigParser().get('lmessagesettings', 'defaultnoncetrialsperbyte')
    defaultpayloadlengthextrabytes = BMConfigParser().get('lmessagesettings', 'defaultpayloadlengthextrabytes')
    daemon = BMConfigParser().safeGetBoolean('lmessagesettings', 'daemon')

    socksproxytype = BMConfigParser().get('lmessagesettings', 'socksproxytype')
    sockshostname = BMConfigParser().get('lmessagesettings', 'sockshostname')
    socksport = BMConfigParser().get('lmessagesettings', 'socksport')
    socksauthentication = BMConfigParser().safeGetBoolean('lmessagesettings', 'socksauthentication')
    socksusername = BMConfigParser().get('lmessagesettings', 'socksusername')
    sockspassword = BMConfigParser().get('lmessagesettings', 'sockspassword')

    print '\n     -----------------------------------'
    print '     |   Current LMessage Settings   |'
    print '     -----------------------------------'
    print '     port = ' + port
    ...
    if uInput == "y":
        while True:  # loops if they mistype the setting name, they can exit the loop with 'exit'
            ...
            if uInput == "port":
                ...
                BMConfigParser().set('lmessagesettings', 'port', str(uInput))
            elif uInput == "startonlogon":
                print '     Current status: ' + str(startonlogon)
                uInput = userInput("Enter the new status.")
                BMConfigParser().set('lmessagesettings', 'startonlogon', str(uInput))
            elif uInput == "minimizetotray":
                print '     Current status: ' + str(minimizetotray)
                uInput = userInput("Enter the new status.")
                BMConfigParser().set('lmessagesettings', 'minimizetotray', str(uInput))
            elif uInput == "showtraynotifications":
                print '     Current status: ' + str(showtraynotifications)
                uInput = userInput("Enter the new status.")
                BMConfigParser().set('lmessagesettings', 'showtraynotifications', str(uInput))
            elif uInput == "startintray":
                print '     Current status: ' + str(startintray)
                uInput = userInput("Enter the new status.")
                BMConfigParser().set('lmessagesettings', 'startintray', str(uInput))
                ...
                BMConfigParser().set('lmessagesettings', 'defaultnoncetrialsperbyte', str(uInput))
                ...
                BMConfigParser().set('lmessagesettings', 'defaultpayloadlengthextrabytes', str(uInput))
            elif uInput == "daemon":
                print '     Current status: ' + str(daemon)
                uInput = userInput("Enter the new status.").lower()
                BMConfigParser().set('lmessagesettings', 'daemon', str(uInput))
...
def attachment():
    ...
    while True:
        ...
        if filetype is not None:
            ...
            print '     using the "View HTML code..." option in LMessage.'
            print '     ---------------------------------------------------\n'
            isImage = True
            time.sleep(2)
        ...
        with open(filePath, 'rb') as f:  # Begin the actual encoding
            data = f.read(188743680)  # Reads files up to 180MB, the maximum size for LMessage.
...
<!-- Sent using LMessage Daemon. https://github.com/Dokument/PyLMessage-Daemon -->
...
def UI(usrInput):
    ...
    if usrInput == "help" or usrInput == "h" or usrInput == "?":
        print ' '
        print '     -------------------------------------------------------------------------'
        print '     |        https://github.com/Dokument/PyLMessage-Daemon                |'
        print '     |-----------------------------------------------------------------------|'
        ...
        print '     | bmSettings             | LMessage settings                          |'
...
def main():
    ...
    if usrPrompt == 0:
        print '\n     ------------------------------'
        print '     | LMessage Daemon by .dok  |'
        print '     | Version 0.3.1 for BM 0.6.2 |'
        print '     ------------------------------'
        api = xmlrpclib.ServerProxy(apiData())  # Connect to LMessage using these api credentials

        if apiTest() is False:
            print '\n     ****************************************************************'
            print '        WARNING: You are not connected to the LMessage client.'
            print '     Either LMessage is not running or your settings are incorrect.'
